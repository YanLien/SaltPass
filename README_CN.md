...
- 🔐 **文件加密**（实验性）：使用 AES-256-GCM 加密存储的特征
- 📁 **多种格式**：支持 TOML 和 JSON 存储格式
- ⌨️ **增强输入**：箭头键导航、删除键支持、显示/隐藏切换
...
⚠️ **警告**：加密功能为实验性。如果您忘记了主盐值，您的数据将无法恢复。

**加密存储**（`*.enc` 文件）：
- 使用 AES-256-GCM 加密特征
- 使用 PBKDF2 和每个文件独立的随机盐值从主盐派生加密密钥
- 每次保存使用随机文件盐值，派生出独立的加密密钥
- 为特征数据提供静态加密

**纯文本存储**（推荐）：
- 特征以可读的 TOML/JSON 格式存储
- 更容易备份和查看
- 需要时可以手动编辑
- 没有因忘记密码而导致数据丢失的风险
